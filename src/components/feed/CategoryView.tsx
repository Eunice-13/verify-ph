 ...
 * loading states and error handling (from Dan's merge). Uses RealCategory
 * type to ensure GENERAL never hits the DB.
 *
    ...
    const loadArticles = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setArticles([]);
          setError(
            err instanceof Error ? err.message : "Unable to load articles. Please try again.",
          );
        }
      } finally {
        if (!cancelled) {
